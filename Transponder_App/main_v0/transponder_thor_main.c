...
static size_t 			size 			= /*(sizeof(rmt_item32_t) * TELEGRAM_SIZE )*/2048;
static rmt_item32_t* 	rmt_current 	= NULL;
static rmt_item16_t*	rmt_delay		= NULL;
 ...
 */
	...
	static int MyLaps_CreateTelegram(int channel, rmt_item32_t* item32, int item_num, unsigned char *msg)
{
    int 	i 			= 0,
			j 			= 0;
	int		last_bit	= ((0b010000000 >> ( 0 % 8 )) & msg[ 0 / 8 ])? true : false;				// initialise the n-1 bits
	
	rmt_item16_t*	item= (rmt_item16_t *)item32;
//	int		phase		= false;
...
//				}
				...
				printf("index %d, duration %d, level %d\n",i,item[i].duration,item[i].level);
				i++;
...
/*		if( current_bit != last_bit ) 	// test if previsous bit is the same 
		...
		int	l =	i-1;																	// TODO Check if the decoder read it
...
static void MyLaps_tx_init()
{
    ...
    rmt_tx.tx_config.carrier_en 			= true;//RMT_TX_CARRIER_EN;
...
static void rmt_mylap_tx_task()
{
	...
	rmt_current 		= (rmt_item32_t*) malloc(size);
	...
	i = MyLaps_CreateTelegram(channel, rmt_current , item_num,radio_msg);
	ESP_LOGI(MYLAPS_TAG, "RMT TX DATA");
	rmt_write_items(channel, rmt_current, item_num, true);
	for(;;) 
	{
